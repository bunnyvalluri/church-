# KCM Ministries Church Platform — Code Intelligence System (Platform & Infrastructure Analysis)

Inspired by [Egonex-AI / Understand-Anything](https://github.com/Egonex-AI/Understand-Anything), this document serves as the DevOps and Platform Engineering reference for the **KCM Ministries Church Platform**. It maps high-level business domains, full-stack architectural flows, module dependencies, API structures, data relationships, and operational strategies to understand, debug, optimize, and scale the entire church platform.
...
- **Workflow**:
  ...
  4. Socket.io emits `event:created` to connected clients.
...
- **Workflow**:
  ...
  5. API saves `Sermon` record in Neon Postgres and triggers FCM push notifications.
...
- **Workflow**:
  ...
  2. Trigger event invokes `dispatchEventNotification()`.
...
- **Workflow**:
  ...
  2. `uploadSecurity.ts` validates file extension, mime-type, and max size constraints.
  ...
  5. Public IDs stored in PostgreSQL enable dynamic image transformations on demand.
...
- **Workflow**:
  ...
  5. Sockets push realtime updates to connected browsers.
...
- **Workflow**:
  ...
  3. `httproutes` inspect path prefixes and headers to route traffic to Frontend, Backend API, WebSockets, or Media routes.

### 9. Database Relationship Graph
- **Architecture**: Relational schema in Neon PostgreSQL defined in `database/schema.prisma`.
- **Core Entities**:
  - `User`, `Event`, `Sermon`, `Donation`, `Receipt`, `DeviceToken`, `Branch`, `AgentReachTask`.
...
### Graph 4: Architecture Visualization
```mermaid
graph TB
...
### Graph 9: Realtime Notification Graph
```mermaid
flowchart TD
...
- **Database Read Scaling**:
  - Configure Neon PostgreSQL read replicas for heavy read operations.
- **Gateway API Rate Limiting**:
  - Enforce Gateway API policies (`platform/gateway/policies`) to rate-limit `/api/auth` and `/api/webhooks`.

---
*Documentation maintained by KCM Ministries Software Architecture & SRE Team.*
