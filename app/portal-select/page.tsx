...
export default function PortalSelectPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-slate-900 to-indigo-950 flex flex-col">
      ...
      <header className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-black text-white tracking-tight leading-none">Kingdom of Christ</p>
            <p className="text-[10px] font-semibold text-white/40 uppercase tracking-widest">Ministries Portal</p>
          </div>
        </div>
        <button
          onClick={logout}
          className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/[0.08] text-white/60 hover:text-white text-xs font-semibold transition-all"
        >
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 py-12">
        ...
        <div className="w-full max-w-md mb-10 text-center">
          <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-gradient-to-r ${roleGradientMap[role] || "from-gray-600 to-gray-700"} mb-4`}>
        ...
        <div className="w-full max-w-3xl space-y-4">
            ...
            {allowedPortals.map((portal) => {
              ...
              return (
                <button
                  ...
                  className="group relative bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.08] hover:border-white/20 rounded-3xl p-6 text-left transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl focus:outline-none focus:ring-2 focus:ring-white/20"
                >
                  {/* Gradient glow bg on hover */}
                  <div className={`absolute inset-0 bg-gradient-to-br ${portal.gradient} opacity-0 group-hover:opacity-10 rounded-3xl transition-opacity duration-300`} />

                  {/* Icon */}
                  <div className={`w-14 h-14 bg-gradient-to-br ${portal.gradient} rounded-2xl flex items-center justify-center shadow-xl mb-5 group-hover:scale-110 transition-transform duration-300`}>
          ...
          {blockedPortals.length > 0 && (
            <div className="mt-8">
              <p className="text-[10px] font-extrabold text-white/20 uppercase tracking-widest px-1 mb-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {blockedPortals.map((portal) => {
                  ...
                  return (
                    <div
                      key={portal.id}
                      className="relative bg-white/[0.02] border border-white/[0.04] rounded-2xl p-4 flex items-center gap-4 opacity-40 cursor-not-allowed"
                    >
                      <div className="w-10 h-10 bg-white/5 rounded-xl flex items-center justify-center flex-shrink-0">
                        <Icon className="w-5 h-5 text-white/30" />
                      </div>
                      <div className="min-w-0">
                        <p className="text-sm font-bold text-white/40">{portal.title}</p>
                        <p className="text-xs text-white/20">{portal.subtitle}</p>
                      </div>
                      <Lock className="w-4 h-4 text-white/20 ml-auto flex-shrink-0" />
                    </div>
                  );
                })}
              </div>
        ...
        {/* Quick Role Info */}
        <div className="mt-10 flex items-center gap-2 text-[11px] text-white/25 font-medium">
          <Star className="w-3 h-3" />
          <span>Your access level: <strong className="text-white/40">{roleLabelMap[role]}</strong></span>
        </div>
      </main>
    </div>
  );
