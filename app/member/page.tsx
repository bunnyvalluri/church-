...
const memberTranslations = {
  en: {
    ...
    welcome: "Welcome,",
    signOut: "Sign Out",
    portalDashboard: "Member Portal Dashboard",
    hello: "Hello,",
    ...
    services: {
      ...
      prayers: { title: "Prayer Request Portal", desc: "Submit, edit, and keep track of prayer lines." },
  ...
  te: {
    ...
    services: {
      ...
      prayers: { title: "ప్రార్థన విన్నపాల పోర్టల్", desc: "ప్రార్థన విన్నపాలను సమర్పించండి మరియు ట్రాక్ చేయండి." },
  ...
  hi: {
    ...
    services: {
      ...
      prayers: { title: "प्रार्थना निवेदन पोर्टल", desc: "प्रार्थना निवेदन जमा करें, संपादित करें और ट्रैक करें।" },
...
export default function MemberDashboard() {
  ...
  useEffect(() => {
    async function loadFeeds() {
      if (user?.uid) {
        try {
          // Fetch member statistics (registrations and prayers count)
          ...
          if (announcementsFileRes && announcementsFileRes.ok) {
            ...
            // Read directly from static fallback or mock if GET doesn't exist
            try {
              loadedAnnouncements = [
      ...
      <header className="bg-white/70 dark:bg-gray-900/60 backdrop-blur-md border-b border-gray-150 dark:border-white/5 sticky top-0 z-40 transition-colors">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2">
            <h1 className="text-2xl font-black bg-gradient-to-r from-purple-600 to-indigo-600 bg-clip-text text-transparent tracking-tight">
              {mt.portalName}
            </h1>
            <span className="text-[10px] uppercase font-bold tracking-widest px-2 py-0.5 bg-purple-100 dark:bg-purple-900/50 text-purple-700 dark:text-purple-300 rounded-full">
              {mt.roleName}
            </span>
          </Link>
          <div className="flex items-center gap-4">
            <span className="text-sm font-semibold hidden md:inline">
              {mt.welcome} {user?.name || (language === "te" ? "సభ్యులు" : language === "hi" ? "सदस्य" : "Member")}
            </span>
            <button
              onClick={logout}
        ...
        <div className="grid lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <div className="bg-gradient-to-br from-purple-600 to-indigo-700 rounded-3xl p-8 text-white relative overflow-hidden shadow-xl">
              <div className="absolute top-0 right-0 w-48 h-48 bg-white/10 rounded-full filter blur-2xl transform translate-x-10 -translate-y-10 animate-pulse" />
              ...
              <h2 className="text-4xl font-extrabold tracking-tight mb-3">
                {mt.hello} {user?.name || (language === "te" ? "విశ్వాసి" : language === "hi" ? "विश्वासी" : "Believer")}!
              </h2>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white dark:bg-gray-800/50 p-6 rounded-2xl border border-gray-100 dark:border-gray-700/50 shadow-md">
                <span className="text-gray-400 dark:text-gray-500 text-xs font-bold uppercase tracking-wider block">{mt.registeredEvents}</span>
                <span className="text-3xl font-black text-purple-600 dark:text-purple-400 block mt-1">{stats.events}</span>
              </div>
              <div className="bg-white dark:bg-gray-800/50 p-6 rounded-2xl border border-gray-100 dark:border-gray-700/50 shadow-md">
                <span className="text-gray-400 dark:text-gray-500 text-xs font-bold uppercase tracking-wider block">{mt.prayerRequests}</span>
                <span className="text-3xl font-black text-indigo-600 dark:text-indigo-400 block mt-1">{stats.prayers}</span>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {[
                  { title: mt.services.profile.title, desc: mt.services.profile.desc, href: "/member/profile", icon: User, color: "text-purple-500 bg-purple-50 dark:bg-purple-950/20" },
                  { title: mt.services.events.title, desc: mt.services.events.desc, href: "/member/events", icon: Calendar, color: "text-indigo-500 bg-indigo-50 dark:bg-indigo-950/20" },
                  { title: mt.services.prayers.title, desc: mt.services.prayers.desc, href: "/member/prayers", icon: Heart, color: "text-rose-500 bg-rose-50 dark:bg-rose-950/20" },
                  { title: mt.services.sermons.title, desc: mt.services.sermons.desc, href: "/member/sermons", icon: BookOpen, color: "text-blue-500 bg-blue-50 dark:bg-blue-950/20" },
                  { title: mt.services.volunteer.title, desc: mt.services.volunteer.desc, href: "/member/volunteer", icon: Briefcase, color: "text-amber-500 bg-amber-50 dark:bg-amber-950/20" },
                  { title: mt.services.giving.title, desc: mt.services.giving.desc, href: "/give", icon: Calendar, color: "text-green-500 bg-green-50 dark:bg-green-950/20" },
                ].map((link, idx) => (
                  <Link
                    key={idx}
                    href={link.href}
                    className="group bg-white dark:bg-gray-800/40 rounded-2xl border border-gray-100 dark:border-gray-700/50 shadow-md p-6 hover:shadow-xl hover:border-purple-200 dark:hover:border-purple-900/50 hover:scale-[1.01] transition-all flex items-start gap-4"
                  >
                    <div className={`p-3.5 rounded-xl flex-shrink-0 group-hover:scale-110 transition-transform ${link.color}`}>
                      <link.icon className="w-6 h-6" />
                    </div>
                    <div className="space-y-1">
                      <h4 className="font-bold text-gray-955 dark:text-white flex items-center gap-1 group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">
                        {link.title}
          ...
          <div className="lg:col-span-4 bg-white dark:bg-gray-800/40 rounded-3xl border border-gray-100 dark:border-gray-700/50 shadow-xl p-8">
            <h3 className="text-xl font-bold text-gray-955 dark:text-white mb-6 flex items-center gap-2">
            ...
            {loadingFeeds ? (
              <div className="space-y-4">
                {[1, 2].map((i) => (
                  <div key={i} className="h-20 bg-gray-50 dark:bg-gray-700/30 animate-pulse rounded-xl" />
                ))}
              </div>
            ) : announcements.length === 0 ? (
              <div className="text-center py-8">
                <p className="text-sm text-gray-500 dark:text-gray-400">{mt.noAnnouncements}</p>
              </div>
            ) : (
              <div className="space-y-4">
                {announcements.map((anc) => (
                  <div 
                    ...
                    className={`p-5 rounded-2xl border text-sm leading-relaxed ${
                      anc.priority === 'URGENT' || anc.priority === 'HIGH'
                        ? 'bg-red-50/50 dark:bg-red-950/10 border-red-100 dark:border-red-900/30'
                        : 'bg-gray-50/50 dark:bg-gray-900/20 border-gray-150 dark:border-white/5'
                    }`}
                  >
                    <div className="flex justify-between items-start gap-2 mb-2">
                      <span className="font-bold text-gray-955 dark:text-white block">{anc.title}</span>
                      {anc.priority === 'HIGH' || anc.priority === 'URGENT' ? (
                        <span className="bg-red-500 text-white text-[9px] font-extrabold uppercase px-2 py-0.5 rounded-full">
                          {mt.urgent}
                        </span>
                      ) : null}
                    </div>
