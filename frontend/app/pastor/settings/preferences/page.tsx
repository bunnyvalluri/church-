...
import { Globe, Moon, LayoutGrid, Save } from "lucide-react";
...
export default function PastorPreferencesSettingsPage() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      <PastorPageHeader
        title={t.preferencesSettingsTitle}
        subtitle={t.preferencesSettingsSubtitle}
        badge={t.displayPreferencesBadge}
      />
      ...
      <div className="bg-white/70 dark:bg-[#0E0F24]/70 p-6 sm:p-8 rounded-2xl sm:rounded-3xl border border-slate-200/60 dark:border-white/[0.06] shadow-sm max-w-3xl mx-auto space-y-6">
        ...
        {saved && (
          <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl text-xs font-bold text-emerald-600 dark:text-emerald-400 flex items-center gap-2 animate-in fade-in">
            <span>✓</span> {t.preferencesSavedMsg}
          </div>
        )}
        ...
        <form onSubmit={handleSave} className="space-y-4">
          <div className="flex items-center justify-between p-4 rounded-xl bg-slate-50 dark:bg-white/[0.02] border border-slate-200/60 dark:border-white/5">
            <div className="space-y-0.5">
              ...
              <p className="text-[10px] text-slate-500 dark:text-gray-400">{t.teluguToggleDesc}</p>
            </div>
            <input
              type="checkbox"
              checked={prefs.bilingualToggle}
              onChange={(e) => setPrefs({ ...prefs, bilingualToggle: e.target.checked })}
          ...
          <div className="flex items-center justify-between p-4 rounded-xl bg-slate-50 dark:bg-white/[0.02] border border-slate-200/60 dark:border-white/5">
            <div className="space-y-0.5">
              ...
              <p className="text-[10px] text-slate-500 dark:text-gray-400">{t.darkModeDefaultsDesc}</p>
            </div>
            <input
              type="checkbox"
          ...
          <div className="flex items-center justify-between p-4 rounded-xl bg-slate-50 dark:bg-white/[0.02] border border-slate-200/60 dark:border-white/5">
            <div className="space-y-0.5">
              ...
              <p className="text-[10px] text-slate-500 dark:text-gray-400">{t.compactLayoutDesc}</p>
            </div>
            <input
              type="checkbox"
              checked={prefs.compactLayout}
              onChange={(e) => setPrefs({ ...prefs, compactLayout: e.target.checked })}
              className="w-4 h-4 rounded text-indigo-600 accent-indigo-600 cursor-pointer"
            />
          </div>
