...
export default function UnifiedEventManagementPortal() {
  ...
  if (!mounted) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center">
        <Loader2 className="w-8 h-8 animate-spin text-violet-550" />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 dark:bg-slate-950 text-slate-800 dark:text-slate-150 transition-colors duration-300 flex flex-col pb-16 relative">
        ...
        <div className="flex items-center gap-4">
          ...
          {["SUPER_ADMIN", "ADMIN"].includes(role) && (
            <button 
              ...
              className="px-3.5 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-white/5 dark:hover:bg-white/10 text-xs font-bold transition-all border border-slate-200/60 dark:border-white/10 text-slate-700 dark:text-slate-300 shadow-inner"
            >
              Portal Selection
            </button>
          )}
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={logout}
              className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-rose-500/10 hover:bg-rose-500/25 text-rose-600 dark:text-rose-400 text-xs font-black transition-all border border-rose-500/15"
              title="Log Out"
            >
      ...
      {/* Grid Content Layout */}
      <main className="max-w-7xl mx-auto w-full px-6 mt-8 grid grid-cols-1 lg:grid-cols-4 gap-8 flex-1">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {/* Welcome banner */}
            <div className="md:col-span-2 relative overflow-hidden rounded-3xl p-6 text-white shadow-xl bg-gradient-to-br from-indigo-600 via-purple-700 to-pink-600 border border-white/20 flex flex-col justify-between group min-h-[220px]">
              
              {/* Luminous background blobs */}
              <div className="absolute top-0 right-0 -mr-16 -mt-16 w-48 h-48 rounded-full bg-yellow-300/30 blur-3xl group-hover:bg-yellow-300/40 transition-colors duration-700 pointer-events-none" />
              <div className="absolute bottom-0 left-0 -ml-16 -mb-16 w-48 h-48 rounded-full bg-cyan-400/30 blur-3xl pointer-events-none" />
              
              <div className="absolute -right-6 -bottom-6 opacity-[0.05] group-hover:scale-110 group-hover:rotate-6 transition-transform duration-500 pointer-events-none">
                <Camera className="w-40 h-40" />
              </div>
              <div className="space-y-2 relative z-10">
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-white/10 border border-white/20 text-[9px] font-black uppercase tracking-widest text-violet-100">
                  <Sparkles className="w-3.5 h-3.5 text-yellow-300 animate-pulse" />
                  Operations Console
                </span>
                <h2 className="text-2xl md:text-3xl font-black mt-2 tracking-tight leading-tight bg-gradient-to-r from-white via-violet-100 to-pink-200 bg-clip-text text-transparent">
                  Hello, {getGreetingName(user?.name)}! 🙏
                </h2>
                <p className="text-xs text-violet-100/80 leading-relaxed max-w-md font-medium mt-1">
              ...
              <div className="flex flex-wrap items-center gap-3 mt-6 relative z-10">
                <Link
                  ...
                  className="flex items-center gap-2 px-5 py-3 bg-white text-purple-700 hover:bg-purple-50 rounded-xl text-xs font-black transition-all shadow-lg active:scale-95 hover:shadow-purple-500/20"
                >
                ...
                <Link
                  ...
                  className="flex items-center gap-2 px-5 py-3 bg-white/15 hover:bg-white/25 border border-white/20 rounded-xl text-xs font-black transition-all active:scale-95"
                >
          ...
          {isManagerOrAdmin && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              ...
              >
                ...
                <div className="w-11 h-11 rounded-2xl bg-emerald-500/20 dark:bg-emerald-500/30 flex items-center justify-center text-slate-550 group-hover:scale-110 transition-transform duration-300 relative z-10">
                    ...
                    >
                      ...
                      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 border-b border-slate-100 dark:border-white/5 pb-4">
                        ...
                        <div className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest text-left md:text-right space-y-1 shrink-0">
                          ...
                          <div className="text-[9px] font-bold text-slate-400 dark:text-slate-550 flex items-center gap-1">
                        ...
                        {isManagerOrAdmin && report.status === "PENDING" && (
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              disabled={actionLoadingId === report.id}
                              className="flex items-center gap-1.5 px-4.5 py-2 rounded-xl border border-rose-200 dark:border-rose-900/30 text-rose-600 dark:text-rose-450 text-xs font-bold hover:bg-rose-500/10 transition-all disabled:opacity-50 active:scale-95"
                            >
                            ...
                            <button
                              ...
                              disabled={actionLoadingId === report.id}
                              className="flex items-center gap-1.5 px-4.5 py-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-650 text-white text-xs font-bold hover:from-emerald-500 hover:to-teal-500 shadow-md hover:shadow-emerald-500/10 transition-all disabled:opacity-50 active:scale-95"
                            >
                        ...
                        {isExpanded && (
                          ...
                          >
                            ...
                            {report.volunteerNames.length > 0 && (
                              <div className="space-y-2">
                                <span className="text-[9px] font-black text-slate-405 dark:text-slate-500 uppercase tracking-widest block">Attending Volunteers</span>
                                <div className="flex flex-wrap gap-1.5">
                                  {report.volunteerNames.map((name, idx) => (
                                    <span key={idx} className="bg-violet-500/5 dark:bg-violet-500/10 border border-violet-500/20 px-2.5 py-1 rounded-xl text-[9px] font-bold text-violet-605 dark:text-violet-400">
                                      {name}
                                    </span>
                                  ))}
                                </div>
        ...
        {editingReport && (
          ...
          >
            <motion.div
              ...
              className="bg-white dark:bg-slate-905 border border-slate-200 dark:border-white/10 rounded-3xl p-6 shadow-2xl max-w-2xl w-full my-8 relative overflow-hidden"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Top Banner Accent */}
