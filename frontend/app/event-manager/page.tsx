...
export default function UnifiedEventManagementPortal() {
  ...
  const handleDeleteSermon = async (id: string) => {
    ...
    try {
      // Cookies are sent automatically — middleware validates __kcm_session_role cookie
      ...
      if (res.ok && data.success) {
        setToastMessage({
          ...
          desc: `Successfully deleted sermon from database.`,
        });
  ...
  const clearSeededSermons = async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        setToastMessage({
          title: "🗑️ Seeded Sermons Cleared",
          desc: `Removed ${data.deletedCount} static sermons. The landing page is now fully dynamic!`,
        });
    ...
    try {
      ...
      if (res.ok && data.success) {
        setToastMessage({
          ...
          desc: `Successfully deleted service/event from database.`,
        });
        setTimeout(() => setToastMessage(null), 5000);
        fetchAllServices();
      } else {
  ...
  return (
    <div className="min-h-screen bg-transparent text-slate-800 dark:text-slate-150 transition-colors duration-300 flex flex-col pb-16 relative">
      ...
      {/* Top Header */}
      <header className="sticky top-0 z-30 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-250/80 dark:border-slate-800/80 px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between shadow-sm" style={{ isolation: 'isolate' }}>
      ...
      <AnimatePresence>
        {showManageSermons && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto" onClick={() => setShowManageSermons(false)}>
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
            ...
            >
              ...
              <button
                type="button"
                onClick={() => setShowManageSermons(false)}
                className="absolute top-4 right-4 p-2 bg-slate-100 hover:bg-slate-200 dark:bg-white/5 dark:hover:bg-white/10 rounded-full text-slate-500 dark:text-slate-400 transition-all cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
              ...
              <div className="flex items-center gap-3 border-b border-slate-100 dark:border-white/5 pb-4 mb-5">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-500 font-bold uppercase mt-0.5 tracking-wider">Delete or update published sermons</p>
                </div>
              </div>
              ...
              <div className="space-y-3 max-h-[450px] overflow-y-auto pr-1 custom-scrollbar">
                {loadingSermons ? (
                  <div className="py-12 text-center text-slate-500">
                    ...
                    <p className="text-xs font-bold">Loading sermons database...</p>
                  </div>
                  ...
                  <div className="py-12 text-center text-slate-500 border border-dashed border-slate-200 dark:border-white/5 rounded-2xl">
                    <Play className="w-8 h-8 mx-auto mb-2 text-slate-350" />
                    <p className="text-xs font-bold">No sermons found in database.</p>
                  </div>
                ) : (
                  sermonsList.map((sermon) => (
                    <div
                      key={sermon.id}
                      className="flex items-center justify-between p-4 bg-slate-50 dark:bg-white/[0.02] border border-slate-150 dark:border-white/5 rounded-2xl hover:border-purple-500/20 dark:hover:border-purple-500/30 transition-all"
                    >
              ...
              <div className="flex items-center justify-between gap-3 mt-6 pt-4 border-t border-slate-100 dark:border-white/5">
                {/* Bulk-clear seeded data — shown only if seeded sermons still exist */}
                <button
                  type="button"
                  onClick={clearSeededSermons}
                  disabled={clearingSeeded || loadingSermons}
                  className="flex items-center gap-2 px-4 py-2.5 text-xs font-bold text-red-600 dark:text-red-400 bg-red-50 hover:bg-red-100 dark:bg-red-950/20 dark:hover:bg-red-950/40 rounded-xl transition-all border border-red-200 dark:border-red-900/40 disabled:opacity-50 cursor-pointer"
                  title="Remove placeholder sermons seeded from the database"
                >
                  {clearingSeeded ? (
                    <Loader2 className="w-3.5 h-3.5 animate-spin" />
                  ) : (
      ...
      <AnimatePresence>
        {showManageServices && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto" onClick={() => setShowManageServices(false)}>
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
            ...
            >
              ...
              <div className="flex items-center gap-3 border-b border-slate-100 dark:border-white/5 pb-4 mb-5">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-500 font-bold uppercase mt-0.5 tracking-wider">Delete or update scheduled services</p>
                </div>
              </div>
              ...
              <div className="space-y-3 max-h-[450px] overflow-y-auto pr-1 custom-scrollbar">
                {loadingServices ? (
                  <div className="py-12 text-center text-slate-500">
                    ...
                    <p className="text-xs font-bold">Loading services database...</p>
                  </div>
                ) : servicesList.length === 0 ? (
                  <div className="py-12 text-center text-slate-500 border border-dashed border-slate-200 dark:border-white/5 rounded-2xl">
                    <Calendar className="w-8 h-8 mx-auto mb-2 text-slate-350" />
                    <p className="text-xs font-bold">No services found in database.</p>
                  </div>
                ) : (
                    ...
                    >
                      <div className="min-w-0 flex-1 pr-4">
                        ...
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1 text-[10px] text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider">
                          ...
                          <span>•</span>
                          <span>{service.branch?.name || "General"}</span>
                          <span>•</span>
