  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: "Active & Draft", value: stats.total, color: "text-slate-900 dark:text-white", dot: "bg-slate-500" },
          { label: "Published Live", value: stats.published, color: "text-emerald-600 dark:text-emerald-400", dot: "bg-emerald-500" },
          { label: "Drafts Queue", value: stats.drafts, color: "text-amber-600 dark:text-amber-400", dot: "bg-amber-500" },
          { label: "Future Live", value: stats.upcoming, color: "text-indigo-600 dark:text-indigo-400", dot: "bg-indigo-500" },
        ].map((s) => (
          <div key={s.label} className="bg-white dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/80 dark:border-white/[0.06] rounded-2xl p-4 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 dark:border-white/10 pb-1">
        {[
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-t-xl text-xs font-bold transition-all border-b-2 -mb-[2px] ${
                isActive
                  ? "border-indigo-600 text-indigo-600 dark:text-indigo-400 bg-indigo-50/70 dark:bg-indigo-500/10"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          {activeTab === "list" && (
            <div className="space-y-6">
              {/* Filter controls */}
              <div className="bg-white dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/80 dark:border-white/[0.06] rounded-2xl p-4 shadow-sm flex flex-col md:flex-row gap-3">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full h-10 pl-10 pr-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 text-xs font-medium text-slate-900 dark:text-slate-100 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
                  />
                </div>
                ...
                <select
                  ...
                  className="h-10 px-3 rounded-xl bg-slate-50 border border-slate-200 dark:bg-slate-950 dark:border-white/10 text-slate-800 dark:text-white text-xs font-bold focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                >
              ...
              {isLoading ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 animate-pulse">
                  {[1, 2, 3, 4].map((i) => (
                    <div key={i} className="h-72 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl" />
                  ))}
                </div>
              ) : events.length === 0 ? (
                <div className="py-20 text-center bg-white dark:bg-slate-900/60 border border-slate-200/80 dark:border-white/[0.06] rounded-2xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  {events.map((event: any) => (
                    <div key={event.id} className="relative group bg-white dark:bg-slate-900/60 border border-slate-200/80 dark:border-white/[0.06] rounded-2xl p-5 shadow-sm space-y-4 hover:shadow-md transition-all flex flex-col justify-between">
                      <div>
                        {/* Status Label */}
                        <div className="flex items-center justify-between mb-2">
                          <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider ${
                            event.status === "PUBLISHED" ? "bg-emerald-50 text-emerald-700 border border-emerald-200/60 dark:bg-emerald-500/15 dark:text-emerald-400 dark:border-transparent" :
                            event.status === "DRAFT" ? "bg-amber-50 text-amber-700 border border-amber-200/60 dark:bg-amber-500/15 dark:text-amber-400 dark:border-transparent" :
                            "bg-slate-100 text-slate-700 border border-slate-200 dark:bg-slate-500/15 dark:text-slate-400 dark:border-transparent"
                          }`}>
                            {event.status}
                          </span>
                          <span className="text-[10px] text-slate-500 dark:text-slate-400 font-bold">{event.branch?.name || "General"}</span>
                        </div>
                        {event.image && (
                          <img src={event.image} alt={event.title} className="h-32 w-full object-cover rounded-xl mb-3 border border-slate-100 dark:border-white/5" />
                        )}
                        <h3 className="text-sm font-black text-slate-900 dark:text-white line-clamp-1">{event.title}</h3>
                        <p className="text-xs text-slate-600 dark:text-slate-300 line-clamp-2 mt-1">{event.shortDescription || event.description}</p>

                        <div className="text-[11px] text-slate-500 dark:text-slate-400 space-y-1 mt-3">
                      ...
                      <div className="flex flex-wrap gap-2 pt-3 border-t border-slate-100 dark:border-white/5">
                        <button
                          ...
                          className="flex-1 h-8 rounded-lg bg-slate-100 dark:bg-white/5 border border-slate-200/80 dark:border-white/10 text-[10px] font-black text-slate-700 dark:text-slate-300 flex items-center justify-center gap-1 hover:bg-slate-200/80 dark:hover:bg-white/10 hover:text-slate-900 transition-all"
                        >
                        ...
                        <button
                          ...
                          className="flex-1 h-8 rounded-lg bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-500/15 dark:hover:bg-indigo-500/25 text-indigo-700 dark:text-indigo-300 border border-indigo-200/70 dark:border-indigo-500/20 text-[10px] font-black flex items-center justify-center gap-1 transition-all"
                        >
                        ...
                        <button
                          ...
                          className="h-8 px-2.5 rounded-lg bg-rose-50 hover:bg-rose-100 dark:bg-rose-500/15 dark:hover:bg-rose-500/25 text-rose-700 dark:text-rose-300 border border-rose-200/70 dark:border-rose-500/20 text-[10px] font-bold flex items-center justify-center transition-all"
                        >
          ...
          {activeTab === "reorder" && (
            <div className="bg-white dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/80 dark:border-white/[0.06] rounded-2xl p-5 shadow-sm space-y-4">
              <div>
                <h3 className="text-sm font-black text-slate-900 dark:text-white flex items-center gap-2">
                  <ChevronUp className="w-4 h-4 text-indigo-500" /> Landing Page Presentation Order
                </h3>
                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Use Up and Down controls to sequence events. The order maps to display priority on the landing page.</p>
              </div>

              <div className="space-y-2 max-h-[70vh] overflow-y-auto pr-1">
                {(allEventsData?.events || []).map((e: any, index: number) => (
                  <div key={e.id} className="flex items-center justify-between p-3.5 bg-slate-50 dark:bg-slate-950/60 border border-slate-200/80 dark:border-white/10 rounded-xl">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="text-xs font-bold text-slate-900 dark:text-white leading-none">{e.title}</h4>
                        <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-1">📅 {new Date(e.date).toLocaleDateString("en-IN")} · {e.location}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <button
                        ...
                        disabled={index === 0}
                        className="p-1.5 rounded-lg bg-white dark:bg-white/5 border border-slate-200/80 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:text-white disabled:opacity-30"
                      >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/80 dark:border-white/[0.06] rounded-2xl p-5 shadow-sm space-y-4">
                <div>
                  ...
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Simulate ticket scanning by entering a Registration ID or Code (e.g. `KCM-TICKET-[id]`).</p>
                </div>

                <form onSubmit={handleCheckin} className="space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black uppercase tracking-wider text-slate-700 dark:text-slate-300">Select Event *</label>
                    <select
                      value={checkinEventId}
                      onChange={(e) => setCheckinEventId(e.target.value)}
                      className="w-full h-11 px-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 text-xs font-bold text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                    >
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black uppercase tracking-wider text-slate-700 dark:text-slate-300">Ticket QR Data or Registration ID *</label>
                    <input
                      value={ticketCode}
                      onChange={(e) => setTicketCode(e.target.value)}
                      placeholder="e.g. clrgwq... or KCM-TICKET-clrg..."
                      className="w-full h-11 px-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 text-xs font-medium text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                    />
                  </div>
                  ...
                  {checkinMessage && (
                    <div className={`p-3.5 rounded-xl text-xs flex items-center gap-2 border ${
                      checkinMessage.type === "success"
                        ? "bg-emerald-50 dark:bg-emerald-950/20 text-emerald-700 dark:text-emerald-400 border-emerald-200/60"
                        : "bg-rose-50 dark:bg-rose-950/20 text-rose-700 dark:text-rose-400 border-rose-200/60"
                    }`}>
                  ...
                  <button
                    ...
                    className="w-full h-11 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 text-white text-xs font-bold shadow-md hover:from-indigo-500 hover:to-violet-500 transition-all disabled:opacity-50 active:scale-95"
                  >
                    Check In Attendee
                  </button>
                </form>
              ...
              {/* Checkin live logs */}
              <div className="bg-white dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/80 dark:border-white/[0.06] rounded-2xl p-5 shadow-sm space-y-3">
                ...
                <div className="space-y-2.5 max-h-[50vh] overflow-y-auto">
                  {liveActivity.filter((act) => act.type === "upload").length === 0 ? (
                    <p className="text-xs text-slate-400 dark:text-slate-500 text-center py-8">No check-ins recorded in this session.</p>
                  ) : (
                    liveActivity.filter((act) => act.type === "upload").map((act) => (
                      <div key={act.id} className="p-2.5 bg-slate-50 dark:bg-slate-950/60 border border-slate-200/80 dark:border-white/5 rounded-xl text-xs font-medium text-slate-800 dark:text-slate-200">
                        {act.text} <span className="float-right text-[10px] text-slate-400 font-bold">{act.time}</span>
                      </div>
                    ))
                  )}
                </div>
          ...
          {activeTab === "reports" && (
            <div className="bg-white dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/80 dark:border-white/[0.06] rounded-2xl p-5 shadow-sm space-y-4">
              <div>
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Submit offerings, visitor metrics, and expenses to auto-complete this event and publish statistics.</p>
              </div>

              <form onSubmit={handleReportSubmit} className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase tracking-wider text-slate-700 dark:text-slate-300">Choose Completed Event *</label>
                  <select
                    value={reportEventId}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black uppercase tracking-wider text-slate-700 dark:text-slate-300">Attendance Count</label>
                    <input type="number" value={attendanceCount} onChange={(e) => setAttendanceCount(Number(e.target.value))} className="w-full h-11 px-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 text-xs font-bold text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20" />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black uppercase tracking-wider text-slate-700 dark:text-slate-300 flex items-center gap-0.5"><DollarSign className="w-3 h-3 text-emerald-500" /> Offering Amount (INR)</label>
                    <input type="number" value={offeringAmount} onChange={(e) => setOfferingAmount(Number(e.target.value))} className="w-full h-11 px-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 text-xs font-bold text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20" />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black uppercase tracking-wider text-slate-700 dark:text-slate-300">New Visitors</label>
                    <input type="number" value={visitorsCount} onChange={(e) => setVisitorsCount(Number(e.target.value))} className="w-full h-11 px-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 text-xs font-bold text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20" />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black uppercase tracking-wider text-slate-700 dark:text-slate-300">Expenses (INR)</label>
                    <input type="number" value={expenses} onChange={(e) => setExpenses(Number(e.target.value))} className="w-full h-11 px-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 text-xs font-bold text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20" />
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black uppercase tracking-wider text-slate-700 dark:text-slate-300">New Saved Members</label>
                    <input type="number" value={newMembersCount} onChange={(e) => setNewMembersCount(Number(e.target.value))} className="w-full h-11 px-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 text-xs font-bold text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20" />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black uppercase tracking-wider text-slate-700 dark:text-slate-300">Prayer Requests Collected</label>
                    <input type="number" value={prayerRequestsCount} onChange={(e) => setPrayerRequestsCount(Number(e.target.value))} className="w-full h-11 px-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 text-xs font-bold text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20" />
                  </div>
                </div>

                <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase tracking-wider text-slate-700 dark:text-slate-300 flex items-center gap-1"><PenTool className="w-3.5 h-3.5" /> Event Summary (Full Report) *</label>
                  <textarea
                    rows={4}
                    value={summary}
                    onChange={(e) => setSummary(e.target.value)}
                    placeholder="Provide a detailed summary of how the event went, testimonies, sermon highlights..."
                    className="w-full p-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 text-xs font-semibold text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 resize-none"
                  />
                </div>
                ...
                <button
                  ...
                  className="w-full h-11 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 text-white text-xs font-bold shadow-md hover:from-emerald-500 hover:to-teal-500 transition-all disabled:opacity-50 active:scale-95"
                >
          ...
          {activeTab === "trash" && (
            <div className="bg-white dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/80 dark:border-white/[0.06] rounded-2xl p-5 shadow-sm space-y-4">
              <div>
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Recover soft-deleted items or manage archived historical events.</p>
              </div>
              ...
              {isLoading ? (
                ...
                <div className="space-y-3">
                  {(allEventsData?.events || []).filter((e: any) => e.isDeleted || e.status === "ARCHIVED").map((e: any) => (
                    <div key={e.id} className="flex items-center justify-between p-3.5 bg-slate-50 dark:bg-slate-950/60 border border-slate-200/80 dark:border-white/10 rounded-xl">
                      <div>
                        <span className={`inline-block px-2 py-0.5 rounded text-[8px] font-black uppercase mb-1 ${e.isDeleted ? "bg-rose-50 text-rose-700 border border-rose-200/60 dark:bg-rose-500/15 dark:text-rose-400 dark:border-transparent" : "bg-slate-100 text-slate-600 border border-slate-200 dark:bg-slate-500/15 dark:text-slate-400 dark:border-transparent"}`}>
                          {e.isDeleted ? "DELETED" : "ARCHIVED"}
                        </span>
                        <h4 className="text-xs font-bold text-slate-900 dark:text-white">{e.title}</h4>
                        <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">Venue: {e.location}</p>
                      </div>
                      <div className="flex gap-2">
                        {e.isDeleted ? (
                          <button
                            onClick={() => handleRestore(e.id)}
                            className="h-8 px-3 rounded-lg bg-indigo-600 text-white text-[10px] font-black flex items-center gap-1 hover:bg-indigo-500 transition-all shadow-sm"
                          >
        ...
        <div className="space-y-5">
          <div className="bg-white dark:bg-slate-900/60 backdrop-blur-md border border-slate-200/80 dark:border-white/[0.06] rounded-2xl p-5 shadow-sm">
            <div className="flex items-center gap-2 border-b border-slate-100 dark:border-white/5 pb-3 mb-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-[10px] text-slate-400 font-semibold">WebSockets Live Monitor</p>
              </div>
            ...
            <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-1">
              {liveActivity.map((act) => (
                <div key={act.id} className="text-xs space-y-0.5 border-l-2 border-slate-200 dark:border-white/10 pl-3.5 relative">
                  ...
                  <p className="font-semibold text-slate-700 dark:text-slate-300 leading-relaxed">{act.text}</p>
